...
async fn run_command(
    ...
    match command {
        Commands::Scan => {
            ...
            let mut current_scan = db::scans_table::insert_scan(&pool).await?;
            ...
            let result: Result<(), Box<dyn std::error::Error>> = async {
                ...
                let cpe_ids = db::cpe_tables::upsert_cpes(&pool, &cpe_entries).await?;

                let (cves, associations) = get_cves(&cpe_ids, client).await;
                for cve in &cves {
                    debug!("CVE: {}", cve.id);
                    db::cve_tables::upsert_cve(&pool, cve).await?;
                }
                ...
                db::findings_table::insert_findings(&pool, &findings).await?;

                let symbols = extract_symbols(cves, client).await;
                for symbol in &symbols {
                    db::symbol_tables::insert_symbol(&pool, symbol).await?;
                }
            ...
            db::scans_table::update_scan(&pool, &current_scan).await?;
            result?;
        }
        ...
        Commands::Inventory => {
            ...
            let resolver = CpeResolver::new(Arc::clone(client));
            let cpe_entries = resolver.resolve_all(&inventory.items).await;
            db::cpe_tables::upsert_cpes(&pool, &cpe_entries).await?;
        }
        ...
        Commands::Symbols { cve_id } => {
            if let Some(cve_id) = cve_id {
                ...
                let pool = db::connect().await?;
                let cves = db::cve_tables::get_all_cves(&pool).await?;
                let symbols = extract_symbols(cves, client).await;
                error!("Extracted {} symbols", symbols.len());
                for symbol in &symbols {
                    db::symbol_tables::insert_symbol(&pool, symbol).await?;
