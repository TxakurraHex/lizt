...
fn main() {
    ...
    let bpf_manifest = manifest_dir.join("../lizt_ebpf_programs/Cargo.toml");
    ...
    let src = target_dir.join("bpfel-unknown-none/release/lizt_ebpf_programs");
    let dst = out_dir.join("lizt_ebpf_programs");
    std::fs::copy(&src, &dst).expect("failed to copy BPF binary to OUT_DIR");

    println!("cargo:rerun-if-changed=../lizt_ebpf_programs");
}
